...
Unquoted searches recognize narrow artifact-type nouns only when a query ends in one unambiguous type noun and includes at least two high-signal terms: `runbook`, `skill`, `doc`/`document`/`documentation`/`reference`, and `memory` (including their exact plural forms) join the existing script, cron/job, and MCP/wrapper intent. At most one fully matching artifact is promoted, only when one query term matches a configured entity that distinguishes one candidate family and another supplies the operation/topic; an owning skill stays adjacent to a promoted support document, and the eligible support sibling is selected before per-parent diversity is applied. Topic-only, mixed, underspecified, or conversational uses of those nouns retain baseline ranking. Quoted searches, explicit type filters, script-reference protection, skill-parent lifting, and support-document diversity retain their normal behavior.
