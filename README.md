...
Before any detached model call, the **entire baseline** must fit the existing bounded Markdown reader. Unsupported, missing, unreadable, oversized or over-budget items close the case with an explicit ineligible reason and zero model calls—even if they might be irrelevant. This intentionally conservative scope restriction never drops baseline entries or invents `not_useful` judgments. Private bounded structural diagnostics and the read-only `routing-report --json` distinguish eligibility refusals, model abstention categories and runtime failures; historical missing receipts remain unknown.
