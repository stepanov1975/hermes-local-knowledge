...
Tool OKFs are small routing artifacts for tools Hermes has actually used. They are not full skills and are not generated from raw tool output. The queue stores only privacy-safe metadata: tool name, toolset, a sanitized schema view, counters, error class, and argument shape. Schema values such as defaults, examples, descriptions, titles, summaries, `$comment`, and secret-like text are redacted before storage. The structured LLM generation packet is narrower still: it omits usage/error counters and receives only the tool identity, toolset, sanitized schema/argument structure, and a bounded `allowed_related_tools` list drawn from already observed tools in the same toolset.

The generator treats candidates independently even when two are processed in one model call. `when_not_to_use` is reserved for supported near-neighbor boundaries and may be empty; `related_tools` may contain only exact identifiers from the candidate's allowlist. That bounded same-toolset allowlist is frozen into the claim and validation checks the exact snapshot supplied to the model, so concurrent usage/ranking changes cannot silently broaden or invalidate it. Both fields remain available for human context and graph edges, but neither is indexed as positive lexical search text. Generated body prose is positive-purpose-only human context and is also excluded from trigger, entity, and FTS extraction; positive routing evidence comes from the structured tool identity, title, aliases, and triggers. Generated toolset metadata is deterministic rather than model-authored.
...
The validator requires generated OKFs to live under `<state_dir>/okfs/tools`, use `.md`, declare `artifact_type: tool_okf`, carry the current `generator_version`, match the claimed tool/toolset/schema hash/target path, contain useful positive routing aliases or triggers, constrain related tools to the exact claim-time allowlist snapshot, and avoid obvious secret assignments, generic privacy boilerplate, and transient runtime counters. Increasing `OKF_GENERATOR_VERSION` requeues completed candidates once while preserving pending, claimed, and terminal-error lifecycle state; active older claims retain their old generator identity and cannot complete as current. Schema or null-safe toolset changes also requeue completed candidates and clear stale claim/output state. Existing local files remain available until their bounded regeneration succeeds. `index.sqlite` records an independent index-format version; ordinary lookups rebuild missing, corrupt, or older-format indexes before searching, while a newer format is rejected with expected/actual versions so an older runtime cannot overwrite it. SQLite publication retries brief `PermissionError` failures caused by active Windows readers; if the bounded retries expire, the prior index remains intact. Completing an OKF adds a token under `okf_index_dirty/`; the next normal native or configuration-backed CLI (`--from-hermes-config`) `search`, `get`, or `neighbors` lookup rebuilds the configured default index and removes only the tokens covered by that successful build. Standalone CLI lookups without `--from-hermes-config` reuse the existing index but never rebuild shared default state or consume its dirty tokens, because their current-directory source has no verified relationship to that index. An explicit CLI `--db` is always treated as caller-owned and is never rebuilt automatically, even when its filename is `index.sqlite`. Tokens added concurrently remain for the following lookup. Every native, CLI, doctor, and compatibility-wrapper index publisher participates in the same reentrant OS advisory lock on `index_build.lock` (`flock` on POSIX and a byte-range lock on Windows), so an older scan cannot overwrite a newer index and consume its dirty tokens. Nested same-thread compatibility wrappers reuse the held lock; other threads and processes wait. POSIX fork children close inherited lock descriptors and clear inherited ownership before continuing. The lock file may remain while idle; the OS releases ownership automatically if a builder exits.
