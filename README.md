...
Unquoted searches recognize narrow artifact-type nouns only when a query ends in one unambiguous type noun and includes at least two high-signal service/project and operation/topic terms: `runbook`, `skill`, `doc`/`document`/`documentation`/`reference`, and `memory` (including their exact plural forms) join the existing script, cron/job, and MCP/wrapper intent. At most one fully matching artifact is promoted, and at least one non-generic term must match that artifact's ID, title, path, or declared entities; an owning skill stays adjacent to a promoted support document. Generic operational words alone do not count as service/project evidence. Mixed, underspecified, or conversational uses of those nouns retain baseline ranking. Quoted searches, explicit type filters, script-reference protection, skill-parent lifting, and support-document diversity retain their normal behavior.
