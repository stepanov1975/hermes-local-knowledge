...
Unquoted searches recognize narrow artifact-type nouns only when a query ends in one unambiguous type noun and includes at least two high-signal terms: `runbook`, `skill`, `doc`/`document`/`documentation`/`reference`, and `memory` (including their exact plural forms) join the existing script, cron/job, and MCP/wrapper intent. At most one fully matching artifact family is promoted. The exact target must carry a complete configured entity label in its own ID, title, or path and must match a distinct operation/topic term; body-only mentions and evidence from support siblings do not qualify. Eligibility is resolved across the complete current index, then one stable move keeps an owning skill adjacent to a promoted support document without reordering unrelated results. Topic-only, mixed, underspecified, ambiguous, or conversational uses of those nouns retain baseline ranking exactly. Quoted searches, explicit type filters, script-reference protection, skill-parent lifting, and support-document diversity retain their normal behavior.
...
`knowledge_usage_report` summarizes recent activity before changing ranking, triggers, source coverage, or graph edges. Its `current_native_search_quality` headline isolates live native searches from the current plugin version while excluding known probe queries. The nested `implicit_consumed_rank_lower_bound` aggregates where observed implicit consumption fell in each search's unassisted `baseline_top_ids_json`; it is a lower-bound operational diagnostic, never evaluation ground truth. `event_cohorts` reports current searches, probes, hourly doctor runs, other CLI/native activity, and historical-version searches separately; `implicit_feedback_by_consumer` distinguishes learned `knowledge_get`, `skill_view`, and `read_file` signals from migrated legacy rows. The original aggregate fields remain available for operations.
