...
The writer fails closed on Windows because this dependency-free script cannot guarantee a restrictive DACL.
...
The `compare` command first replays the bound frozen index and requires an exact match with the supplied incumbent rankings. It then generates the candidate internally from the bound authority input; it never accepts an externally supplied candidate ordering. The report binds the exact benchmark, verified baseline, authority, and generated candidate hashes, then compares acceptable Hit@1/3/5/10 and MRR@10, canonical-current top-1, harmful top-1, unjudged top-1, and `none_needed` behavior. Exact-ID metrics remain authoritative, while companion parent-equivalent acceptable-hit, MRR, and unjudged metrics relate only a `skill_support_doc` to its one unambiguous owning skill as recorded in that same frozen index; arbitrary graph neighbors and support siblings are never equivalent. Every changed top-1 case still requires manual inspection. A report never produces an automatic release verdict.
