...
- Expanded artifact-type intent with query-terminal, single-noun promotion for runbooks, skills, docs/references, and memory documents. At most one fully matching artifact moves forward, only when a configured entity distinguishes one candidate family and another term supplies the operation/topic; the eligible support sibling is selected before per-parent diversity. Topic-only, mixed, underspecified, and conversational noun uses retain baseline ranking. Added a lower-bound implicit consumed-rank diagnostic based on unassisted search baselines.
