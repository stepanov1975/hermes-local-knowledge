...
def _okf_generated_at(frontmatter: dict[str, Any]) -> str | None:
        ...
        try:
            ...
            if clean.startswith("{") and clean.endswith("}"):
                ...
                for item in clean[1:-1].split(","):
                    ...
                    parsed[key.strip().strip("'\"")] = _parse_frontmatter_scalar(value)
                generated = parsed
            else:
                generated = None
    if isinstance(generated, dict):
        ...
        if generated_at:
            return generated_at
    legacy_generated_at = str(frontmatter.get("generated_at") or "").strip()
    return legacy_generated_at or None
...
def _tool_okf_candidates(okf_root: Path | None, root: Path, settings: ScannerSettings) -> list[_Candidate]:
    ...
    for path in _iter_files(
        ...
        artifact = Artifact(
            ...
            updated_at=_okf_generated_at(frontmatter),
