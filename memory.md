...
- Detached model work requires the entire baseline to fit the existing registered-Markdown reader before any model call. Unsupported/oversized/unreadable or over-budget items veto the whole case even when potentially irrelevant: this conservative scope restriction never labels them `not_useful` or drops unknown coverage. Bounded structural diagnostics retain no additional prose/source content, and absent historical receipts remain unknown.
...
- `implicit.py` owns strict same-turn search-result consumption attribution for default-enabled implicit evidence.
