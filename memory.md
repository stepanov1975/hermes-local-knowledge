...
- Hermes plugin hooks are invoked synchronously. `on_session_finalize` is the correct boundary trigger, but it must only perform a read-only, tightly timeout-bounded check of the durable queue for pending candidates or recoverable stale claims and launch a detached worker; checking only `pending` rows can strand a crashed claimed batch, while a normal SQLite helper can block behind the publication write lock, and performing `ctx.llm` generation inline blocks `/new`, `/reset`, session expiry, and CLI exit. Run generation through a fresh `hermes local-knowledge okf-worker` plugin CLI process so the child receives its own host-owned `ctx.llm` facade while the deterministic renderer, validator, claims, and retries remain plugin-owned. Renew the singleton lease while `ctx.llm` is running because the configured timeout is per provider request and Hermes retries/fallbacks can outlive a one-shot lease. Render and prevalidate each item in a worker-unique temporary file, then hold a short SQLite write transaction while revalidating lease and claim ownership, replacing and validating the canonical file again, and completing or rolling back the claim. Stale recovery must complete a valid still-claimed canonical artifact before applying the retry cap so hard process death converges without accepting unvalidated content.
...
- `when_not_to_use`, `related_tools`, and arbitrary body prose are negative/graph or human-context metadata. Do not add them to positive FTS text, trigger extraction, or entity extraction; generated body prose must be positive-purpose-only, while positive routing evidence comes from structured tool identity, title, aliases, and triggers. Persist an index-format version and make ordinary lookups rebuild older or corrupt indexes when scanner semantics change.
- Classify persisted indexes as missing, corrupt, older, current, or newer. Rebuild only missing/corrupt/older indexes; refuse newer formats before every publisher so an old runtime cannot downgrade derived state. Retry transient SQLite `PermissionError` publication failures for short-lived Windows readers, but preserve the old index and propagate the error after a bounded retry window.
